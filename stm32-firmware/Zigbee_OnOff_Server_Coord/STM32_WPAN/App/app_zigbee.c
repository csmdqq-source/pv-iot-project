...
static uint32_t APP_ZIGBEE_ComputePowerX10(uint16_t voltage_dv, uint32_t current_ma, uint32_t power_w);
...
static uint8_t g_tuya_temperature_dp_id = 131U;     /* 主温度 DP，当前日志最像温度的是 131 -> 30.7C */
static uint8_t g_tuya_temperature_dp_alt1 = 101U;   /* 备用温度 DP 1 */
static uint8_t g_tuya_temperature_dp_alt2 = 133U;   /* 备用温度 DP 2 */
static uint8_t g_tuya_temperature_scale = 10U;      /* 10=原始值单位 0.1°C，1=原始值单位 1°C */
static bool g_tuya_temperature_auto_detect = true;  /* true=允许备用 DP 自动接管 */
/* USER CODE END PV */
...
uint16_t tuya_voltage_dv;    // 鐢靛帇锛屽崟浣嶏細0.1V (decivolt)
uint32_t tuya_current_ma;    // 鐢垫祦锛屽崟浣嶏細mA
uint32_t tuya_power_w;       // 鍔熺巼锛屽崟浣嶏細W
uint8_t tuya_query_timer_id; // 瀹氭椂鍣↖D锛岀敤浜庡懆鏈熸€ф煡璇㈠姛鐜囨暟鎹?
...
bool tuya_temperature_valid;    // 是否已收到真实温度
uint8_t tuya_temperature_dp_id; // 当前实际生效的温度 DP ID，便于调试
...
static uint32_t APP_ZIGBEE_ComputePowerX10(uint16_t voltage_dv, uint32_t current_ma, uint32_t power_w)
{
  if (voltage_dv != 0U && current_ma != 0U) {
    return (uint32_t)((((uint64_t)voltage_dv * (uint64_t)current_ma) + 500U) / 1000U);
  }
  return power_w * 10U;
}
...
static void ParseTuya_TemperatureData(uint8_t dp_id, const uint8_t *data, uint8_t len)
{
  ...
  if (g_tuya_temperature_scale == 0U) {
    APP_DBG("ERROR: Temperature scale cannot be 0");
    return;
  }

  temp_raw = APP_ZIGBEE_TuyaReadUintBE(data, len);

  if (g_tuya_temperature_scale == 10U) {
    temp_x10_raw = temp_raw;
  } else if (g_tuya_temperature_scale == 1U) {
    temp_x10_raw = temp_raw * 10U;
  } else {
    temp_x10_raw = (temp_raw * 10U) / g_tuya_temperature_scale;
  }

  if (temp_x10_raw > 0xFFFFU) {
    temp_x10_raw = 0xFFFFU;
  }

  zigbee_app_info.tuya_temperature_x10 = (uint16_t)temp_x10_raw;
  zigbee_app_info.tuya_temperature_valid = true;
  zigbee_app_info.tuya_temperature_dp_id = dp_id;

  APP_DBG("[TEMP] DP=%u Raw=%lu -> %u.%uC",
          dp_id,
          (unsigned long)temp_raw,
          zigbee_app_info.tuya_temperature_x10 / 10U,
          zigbee_app_info.tuya_temperature_x10 % 10U);
}

static bool IsTuya_TemperatureDP(uint8_t dp_id)
{
  if (dp_id == g_tuya_temperature_dp_id) {
    return true;
  }

  if (zigbee_app_info.tuya_temperature_valid &&
      dp_id == zigbee_app_info.tuya_temperature_dp_id) {
    return true;
  }

  if (!g_tuya_temperature_auto_detect) {
    return false;
  }

  return (dp_id == g_tuya_temperature_dp_alt1) ||
         (dp_id == g_tuya_temperature_dp_alt2);
}
...
static void APP_ZIGBEE_MqttUploadJson(void)
{
  ...
  uint32_t temperature_int = temperature_x10 / 10U;
