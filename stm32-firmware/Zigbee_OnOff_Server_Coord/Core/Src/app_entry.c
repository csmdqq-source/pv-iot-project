...
//    HAL_EXTI_GenerateSWI(&exti_handle);
  ...
  else if (strcmp((char const*)CommandString, "ON") == 0 ||
           strcmp((char const*)CommandString, "CLOSE") == 0)
  {
    APP_DBG("ON OK");
    APP_ZIGBEE_OnOffSend(1U);
  }
  else if (strcmp((char const*)CommandString, "OFF") == 0 ||
           strcmp((char const*)CommandString, "OPEN") == 0)
  {
    APP_DBG("OFF OK");
    APP_ZIGBEE_OnOffSend(0U);
  }
  else if (strncmp((char const*)CommandString, "TIME:", 5) == 0 ||
           strncmp((char const*)CommandString, "TIME=", 5) == 0)
  {
    const char *p = (const char *)CommandString + 5;
    uint32_t unix_time = (uint32_t)strtoul(p, NULL, 10);
    if (unix_time > 0U) {
      APP_ZIGBEE_SetUnixTime(unix_time);
      APP_DBG("TIME OK: %lu", (unsigned long)unix_time);
    } else {
      APP_DBG("TIME INVALID: %s", CommandString);
    }
  }
  else
  {
